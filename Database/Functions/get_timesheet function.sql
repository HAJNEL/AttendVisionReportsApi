...
-- FUNCTION: public.get_timesheet(date, date, text, text, uuid)
-- Legacy overload without p_employee_type; not called by the API, kept for reference.
-- Has the same leave-day Total Span gap as the overload above, unpatched here since it's unused.

-- DROP FUNCTION IF EXISTS public.get_timesheet(date, date, text, text, uuid);
...
CREATE OR REPLACE FUNCTION public.get_timesheet(
	...
	p_employee_id text,
	p_user_id uuid)
    RETURNS TABLE(person text, status text, employee_id text, department text, date text, first_entry text, last_entry text, hours_worked double precision, break_hours double precision)
    LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
    RETURN QUERY
      ...
      WHERE ar.access_date BETWEEN p_date_from AND p_date_to
        AND (
            (p_dept IS NULL AND
                EXISTS (
                    SELECT 1 FROM department_users du
                    WHERE du.user_id = p_user_id AND du.department_id = d.id
                )
            )
            OR (p_dept IS NOT NULL AND ar.department = p_dept)
        )
    ...
    combined_days AS (
      SELECT
        ...
        arcd.access_date
      FROM all_records arcd
      GROUP BY arcd.person, arcd.employee_id, arcd.department, arcd.department_id, arcd.access_date
      UNION
      SELECT
        lr.full_name::text AS person,
        lr.employee_id::text,
    ...
    SELECT
      ...
      COALESCE((eb.raw_out_dt::time)::text, TO_CHAR(ld.to_time, 'HH24:MI:SS'), '') AS last_entry,
      COALESCE(
        EXTRACT(EPOCH FROM (GREATEST(eb.raw_out_dt, eb.eff_in_dt) - eb.eff_in_dt)) / 3600.0, 0
      )::float8 AS hours_worked,
...
ALTER FUNCTION public.get_timesheet(date, date, text, text, uuid)
    OWNER TO postgres;
