...
-- Adding the status column changes the return signature, so the old
-- definition must be dropped before it can be recreated.
...
BEGIN
    ...
    classified_hours AS (
        ...
        -- Include leave days alongside worked days so leave hours flow into
        -- Normal Hours (Sage's TXT format has no separate leave field).
        -- 'Issue' days (no work, no leave) stay excluded, as before.
        WHERE bt.status = 'Worked' OR bt.status ILIKE '%Leave%'
    ),
    per_day_alloc AS (
        SELECT
            ch.employee_id,
    ...
    sage_layout AS (
        SELECT
            ...
            -- Summarize the employee's days into a single status: their one
            -- status if every day agrees, otherwise 'Mixed'.
            CASE
                WHEN COUNT(DISTINCT per_day_alloc.status) = 1 THEN MAX(per_day_alloc.status)
                ELSE 'Mixed'
            END AS status,
